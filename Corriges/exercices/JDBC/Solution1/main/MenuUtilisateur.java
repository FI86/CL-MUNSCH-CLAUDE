...
public class MenuUtilisateur {
     ...
     * @param u : utilisateur a afficher
     */
    private void afficherUtilisateur(Utilisateur u) {
     ...
     */
    private void lireUtilisateurs() {
        List<Utilisateur> liste = new UtilisateurDAO().lire();
     ...
     */
    private void lireUtilisateurParId() {
        ...
        Utilisateur u = new UtilisateurDAO().lireParId(id);
     ...
     * @return un javaBean Utilisateur a creer
     */
    private Utilisateur saisieUtilisateur() {
     ...
     * @return un javaBean Utilisateur a creer
     */
    private Utilisateur saisieUtilisateur(Utilisateur u) {
        Scanner clavier = new Scanner(System.in);
    ...
    /**
     * Creation d'un utilisteur saisie et valide
     */
    private void creationUtilisateur() {
        Utilisateur u = saisieUtilisateur();
        String erreur = new UtilisateurDAO().validation(u);
        
        if ("".equals(erreur)) {
            if (new UtilisateurDAO().creer(u) == true) {
    ...
    private void supprimerUtilisateur() {
        int id = Utilitaires.verifSaisieInt(new Scanner(System.in), "Entrez l'id de l'utilisateur a supprimer : ", "Erreur : Veuillez saisir un nombre entier : ");
        
        if (new UtilisateurDAO().lireParId(id) != null) {
            if (new UtilisateurDAO().suppressionParId(id) == true) {
                System.out.println("Utilisateur supprime.\n");
            }
        }
        else {
            System.out.println("L'id " + id + " de l'utilisateur a supprimer n'existe pas.\n");
        }
    }
    ...
    private void modifierUtilisateur() {
        int id = Utilitaires.verifSaisieInt(new Scanner(System.in), "Entrez l'id de l'utilisateur a modifier : ", "Erreur : Veuillez saisir un nombre entier : ");
        Utilisateur ul = new UtilisateurDAO().lireParId(id);
        ...
        if (ul != null) {
            ...
            if ("".equals(erreur)) {
                if (new UtilisateurDAO().modifier(new Utilisateur(id, u.getNumero(), u.getNom(), u.getPrenom(), u.getEmail(), u.getLogin(), u.getMdp())) == true) {
                    System.out.println("Utilisateur modifie.\n");
                }
            }
            else System.out.println(erreur);         
        }
        else System.out.println("L'id " + id + " de l'utilisateur a modifier n'existe pas.\n");
    }
