...
public class Dao {
    ...
    public static <C> boolean suppressionParId(Class<C> classeBean, Integer id) {
        String nomTable = nomTable(classeBean);
    ...
    public static <T> boolean creer(T bean) {
        ...
        try(Connection conn = Connexion.connecte(NOM_BDD)) {
            ...
            System.out.println("Erreur lors de la cr�ation de la table " + nomTable.toLowerCase());
            System.out.println(e);
            return false;
        }
    }    
    ...
    private static String testVide(Object obj) {
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                if (valeur instanceof String && ((String) valeur).isEmpty()) messageErreur += "Le champ " + field.getName() + " ne doit pas �tre vide.\n";
                if (valeur instanceof Integer && (Integer) valeur < 0) messageErreur += "Le champ " + field.getName() + " doit �tre un entier positif.\n";
                if (valeur == null) messageErreur += "Le champ " + field.getName() + " ne doit pas �tre nul.\n";
            }
    ...
    public static <T> String validations(T bean) {
        ...
        if (bean instanceof Article article) {
            if (!"F".equals(article.getFc().toUpperCase()) && !"C".equals(article.getFc().toUpperCase())) messageErreur.append("Le champ FC doit �tre F ou C.\n");
        ...
        if (bean instanceof Utilisateur utilisateur) {
            ...
            if (utilisateur.getMdp().length() < 8) messageErreur.append("Le mot de passe doit comporter au moins 8 caract�res.\n");
        }
        
        return messageErreur.toString();
    }    
    
    public static <C> String nomTable(Class<C> classeBean) {
        String nomTable = "";
